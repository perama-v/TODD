...
### Functional high level methods

It is RECOMMENDED that software implementations of the database model provide the following
methods for high level functional operations.

```python
def read_query(query: str) -> List[RecordValue]
    """
    Return the RecordValues that match a user query string.
    """
    return # TODO
```
...
def create_database()
    """
    Creates the database from raw data.
    """
    for c in get_all_chapter_ids():
        for v in get_all_volume_ids():
            save_chapter(get_one_chapter(c, v))
    return
```
...
def extend_database()
    """
    Extends the database using raw data.
    """
    for c in get_all_chapter_ids():
        for v in get_all_volume_ids():
            if missing_chapter(c, v):
                save_chapter(get_one_chapter(c, v))
    return
```

### Helper methods common to all databases


It is RECOMMENDED that software implementations of the database model provide the following
helper methods that apply to all databases.


```python
def get_one_chapter(vol: VolumeId, chapter: ChapterId) -> Chapter:
    """
...
```python
def get_raw_keys(vol: VolumeId, chapter: ChapterId) -> List[RawKeys]:
    """
    Return the raw data relevant to the specified Chapter.
    """
    return # TODO
```

### Helper methods requiring per-database implementation

It is RECOMMENDED that software implementations of the database model provide the following
helper methods for internal operation.

```python
def chapter_to_chapter_id(chapter: Chapter) -> ChapterId:
    """
    Return the ChapterId for a given Chapter.
    """
    return # TODO
```

```python
def volume_to_volume_id(volume: Volume) -> VolumeId:
    """
    Return the VolumeId for a given Volume.
    """
    return # TODO
```

```python
def chapter_matches_ids(chapter: Chapter, vol: VolumeId, chapter: ChapterId) -> bool:
    """
    Determine if a Chapter matches the desired identifiers.
    """
    return # TODO
```
```python
def raw_data_to_record(data: RawData) -> Record:
    """
    Creates a formatted Record from the corresponding raw data.
    """
    return # TODO
```
```python
def get_all_chapter_ids() -> List[ChaterId]
```
```python
def get_all_volume_ids() -> List[VolumeId]
```
```python
def chapter_interface_id(chapter: Chapter) -> str
```
```python
def chapter_interface_id(chapter: Chapter) -> str
```
```python
def record_key_to_chapter_id(key: RecordKey) -> VolumeId
```
### Manifest
